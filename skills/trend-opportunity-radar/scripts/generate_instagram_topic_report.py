...
def build(snapshot: dict[str, Any], analysis: dict[str, Any], language: str) -> dict[str, Any]:
    ...
    complete = bool(
        int((collection.get("counts") or {}).get("detail_open_count") or 0) >= 2
        and len(relevant) >= 4
        and len(counter) >= 1
        and repeatability.get("pass_count", 0) >= 2
    )
    labels = {
        "zh": {
    ...
    return {
        ...
        "research_basis": {
            ...
            "repeat_pass_count": int(repeatability.get("pass_count") or 0),
            "repeat_overlap_jaccard": repeatability.get("overlap_jaccard"),
...
def html_page(report: dict[str, Any]) -> str:
    ...
    stats = [(l["queries"], b["query_count"]), (l["observed"], b["observed_post_count"]), (l["unique"], b["unique_post_count"]), (l["details"], b["detail_post_count"]), (l["relevant"], b["relevant_post_count"]), (l["counter"], b["counter_signal_count"])]
