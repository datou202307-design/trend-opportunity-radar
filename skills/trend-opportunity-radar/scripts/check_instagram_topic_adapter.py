...
SCHEMA_VERSION = "instagram-hashtag-browser-preflight-v0.1"
...
def build_status(probe: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "explicit_hashtag_surface": bool(as_text(probe.get("hashtag")) and as_text(probe.get("query_url"))),
        "canonical_post_links": len({as_text(item) for item in links if as_text(item)}) >= 6,
        ...
        "no_account_search_proxy": probe.get("no_account_search_proxy") is True,
        "no_personalized_explore_feed": probe.get("no_personalized_explore_feed") is True,
    ...
    return {
        ...
        "adapter": "instagram_hashtag_browser_capture",
        "checked_at": now_iso(),
        "ready": ready,
        "status": "ready" if ready else "probe_incomplete",
        "capabilities": {"instagram": ready},
        "research_scopes": {"instagram": ["topic_research"] if ready else []},
        "checks": checks,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate a redacted Instagram hashtag topic-read capability probe.")
    parser.add_argument("--probe", required=True)
    parser.add_argument("--output", required=True)
    args = parser.parse_args()
    probe = load_data(args.probe)
    if not isinstance(probe, dict):
        raise SystemExit("Instagram hashtag browser probe must be a JSON object.")
