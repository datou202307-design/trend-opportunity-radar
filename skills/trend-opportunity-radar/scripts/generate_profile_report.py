...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [f"# {ui['name']}：{subject_name(report)}" if zh else f"# {ui['name']}: {subject_name(report)}", ""]
    if (report.get("demo") or {}).get("synthetic") is True:
        lines.extend([f"> **{'合成演示，不是真实平台结论' if zh else 'Synthetic demo — not a live platform conclusion'}**", ""])
    lines.extend([f"> {ui['question']}", "",
...
def render_html(report: dict[str, Any]) -> str:
    ...
    demo_html = ""
    if (report.get("demo") or {}).get("synthetic") is True:
        demo_label = "合成演示，不是真实平台结论" if zh else "Synthetic demo — not a live platform conclusion"
        demo_html = f'<aside class="demo-banner" role="note">{html.escape(demo_label)}</aside>'
