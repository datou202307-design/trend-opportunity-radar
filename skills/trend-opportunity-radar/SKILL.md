...
Instagram is a scope-specific pilot with separate routes. For a user-supplied public brand, competitor, or creator account under `account_research`, read [instagram-account-adapter.md](references/instagram-account-adapter.md). For `topic_research`, read [instagram-hashtag-topic-adapter.md](references/instagram-hashtag-topic-adapter.md), translate the subject into explicit hashtags across the frozen query layers, and use the hashtag-specific plan/record workflow with `--allow-pilot`. Never route a general topic to account research, or use account search, personalized Explore, the generic Reels feed, Followers, or Following as topic or audience evidence. A displayed hashtag post count is a supply-volume hint, not observed sample count or search demand.
