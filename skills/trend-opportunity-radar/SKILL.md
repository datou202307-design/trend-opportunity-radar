...
For Instagram topic reads, verify the requested URL, the visible hashtag identity, and the page terminal state after every navigation. Zero links alone never prove zero platform results. Record `verified_zero_results` only when two paced passes both expose the target hashtag plus an explicit platform empty state; classify blank pages, shell-only pages, wrong redirects, unclaimed sessions, connection loss, and parser misses as `surface_unreadable`. Retry or fall back internally while preserving proven results. Recovery text must name only the controller actually selected and preflighted for that run; never infer that the Browser plugin is missing from an OpenCLI failure.
...
Facebook `topic_research` is an explicit Beta limited to the Posts search surface in a user-authorized logged-in browser; the internal route remains `pilot` so it always requires explicit opt-in and a live preflight. Read [facebook-topic-adapter.md](references/facebook-topic-adapter.md). Freeze the exact `/search/posts/` query URL, retain only canonical public post or Reel identities, and keep the personalized home Feed, friends, notifications, Marketplace, private groups, and mixed people/page/group search outside the evidence ledger. OpenCLI's generic Facebook search may be used as a capability diagnostic, but its mixed results do not satisfy the topic-sampling contract. After every navigation, verify the Posts-only URL, frozen query identity, and either canonical cards or an explicit platform empty state. A filter skeleton, blank page, wrong redirect, connection loss, or zero-length parser output is `surface_unreadable`, not evidence of zero demand. Do not report a completed Facebook study until at least one retained public result has a verified detail.

On a verified Facebook detail with a positive visible comment total and no comment bodies, the executor may activate that exact detail's numeric Comment control once, then perform at most one delayed read under the shared pacing ledger. Keep no more than five explicitly visible top-level comments and run the normal comment review contract before using them. Never focus or submit the comment composer. CAPTCHA, rate limit, expired login, permission checkpoint, private content, and abnormal redirect are distinct safety stops; none may be converted into zero results.
...
When a verified target surface explicitly returns no visible results, record an empty chunk with `can_continue: false`, empty result and signal arrays, the preserved raw artifact, and `stop_reason: zero_results`. A zero-length parser result without target identity and explicit platform empty evidence is a collection failure, not a zero-result query. Let the orchestrator store `completed_with_zero_results` and calculate the terminal contract decision. Never leave the query `in_progress` or generate a report from an `in_progress` snapshot.
