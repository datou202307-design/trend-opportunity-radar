...
当前版本是 **v0.14.1 candidate**。这一版新增按用户状态区分的安装与首次使用路径、60 秒首次研究入口和双语运行动图，同时保留五场景合成案例库与本地私有研究工作区。外部无辅导 onboarding 验收已延期，因此它仍是候选版，不能称为经过真实用户验证的 onboarding 版本。它不会自动迁移旧报告、创建定时任务，也不预测爆款、流量、需求或收入；首次真实三天监测对比仍待完成。

## 60 秒开始

只选择符合你当前情况的一条路径。已经安装的用户不需要再次下载或安装。

| 你的情况 | 现在怎么做 | 需要 ZIP 吗？ |
|---|---|---|
| **已经安装** | 新建一个 Agent 任务，直接发送下面的首次研究口令。 | 不需要 |
| **一句安装** | 使用下面的跨 Agent 命令从 GitHub 安装；Claude Code 也可以使用[安装](#安装)部分的 Marketplace 命令。 | 不需要 |
| **手动回退** | 从 [Releases](https://github.com/datou202307-design/trend-opportunity-radar/releases) 下载最新审查包，核对校验值，再把唯一的 Skill 文件夹复制到 Agent。 | 需要 |
...
**2. 在新 Agent 任务中开始第一次研究**

```text
使用 trend-opportunity-radar，分析 AI 旅行规划在 YouTube 英语市场的产品需求。
```

Skill 最少只需要一个主题和一个平台；如果你已经知道业务目的，可以一起说明。正式研究只检查目标平台本次实际需要的只读能力，不会安装工具、修改登录状态，也不会要求你提供密码、Cookie 或 Token。确实缺少前提时，只提示一项具体操作，并保留原研究请求供你继续。
...
至少保存过一次研究后再使用工作区。它只索引本地运行清单和报告，不会采集平台数据、上传研究内容或创建定时任务。
...
上面的五步路径由持久化运行清单、不可变阶段收据、原文链接、反例，以及相互独立的观察热度和证据可靠性等级约束。搜索卡片不会因为互动高就自动成为结论；视频字幕、机器转写和 OCR 也会分别标明来源，不会改写成平台事实。
